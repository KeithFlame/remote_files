...
while True:
    ...
    if input == ord('q'):
        break
    if input == ord('w'):
        # str1 = './pic/left/%04d.jpg' % (iter)
        # str2 = './pic/right/%04d.jpg' % (iter)
        str1 = './pics/left/left_%d.jpg' % (iter)
        str2 = './pics/right/right_%d.jpg' % (iter)
        rc1 = cv2.imwrite(str1, frame1)
        rc2 = cv2.imwrite(str2, frame2)
        if rc1 and rc2 :
            str = "The %02dth image has been saved" % (iter)
            print(str)
        iter += 1
