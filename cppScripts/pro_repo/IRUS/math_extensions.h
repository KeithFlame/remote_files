...
namespace Keith {
	...
	Eigen::Matrix3d rpy2Rotation(Eigen::Vector3d);
	...
	Eigen::Vector6d getInverseKinematics(const Eigen::Matrix4d& target, const Eigen::Vector4d& L1r2g,
		const double zeta, Eigen::Vector6d& psi, double& residue);

	const Eigen::Matrix4d calcMiddleMatrix(const Eigen::Matrix4d& cur_T, const Eigen::Matrix4d& tar_T, const double increment);
	const Eigen::Vector6d clacMiddleWrench(const Eigen::Vector6d& cur_W, const Eigen::Vector6d& tar_W, const double increment);
	const Eigen::Vector6d clacMiddleActuation(const Eigen::Vector6d& cur_Psi, const Eigen::Vector6d& tar_Psi, const double increment);
}
