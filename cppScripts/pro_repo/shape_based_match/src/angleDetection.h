...
static std::string prefix = "./../test_img/";
...
int angle_test(std::string mode = "test"){
	...
	if (mode == "train"){
		...
		for (std::string& class_id : class_ids){
			cv::Mat img = cv::imread(prefix + "dualGrapper/" + class_id + ".png");
			if (img.empty())
				std::cout << "image path wrong!" << std::endl;
		...
		detector.writeClasses(prefix + "dualGrapper/%s_templ.yaml");
		...
		std::vector<std::string> class_ids{ "up", "down" };
		detector.readClasses(class_ids, prefix + "dualGrapper/%s_templ.yaml");
		...
		//Timer timer;
		auto matches = detector.match(img, 80, class_ids);
		//timer.out();
