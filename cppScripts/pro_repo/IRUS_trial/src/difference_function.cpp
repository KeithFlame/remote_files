...
{
	double phi = (v_phi_l[4] + v_phi_l[6]);  // Ĭ��0
	...
	double& energy_rsd, Eigen::Matrix4d target)
{
	...
	double lambda(1e-15), eps(1e-6), is_energy(0.000), yita(1e-4), del(1e-9);
	...
	while (iter < max_iter) {
		...
		iter++;
		std::cout << "data collection: " << energy_before <<"    "<< err << std::endl;
		if ((err < eps)) {
			//float t1 = mmath::timer::getDurationSince(tic);
