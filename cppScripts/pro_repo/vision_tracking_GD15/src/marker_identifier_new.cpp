...
static void refineCircleCenters(const cv::Mat gray_img, const cv::Rect& gb_rect, std::vector<std::vector<cv::Point2f>>& markerPoints)
{
	if (markerPoints.size() > 0)
	{
		...
		{
			int half_len = (int)(std::min(cv::norm(markerPoints.at(i).at(0) - markerPoints.at(i).at(4)),
				cv::norm(markerPoints.at(i).at(3) - markerPoints.at(i).at(4))) * 1.5 / 5.5);
			if (half_len > 0)
			{
	...
	std::vector<cv::Point2f>& temp_result)
{
	...
	//	cv::circle(img_color, temp_result[i], 15, cv::Scalar(0, 255, 120), -1);//���㣬��ʵ����ʵ��Բ
	//}
...
void findSquares_adapt(const cv::Mat& image, std::vector<std::vector<cv::Point>>& squares)
{
	...
	cv::adaptiveThreshold(image, image_binary, 255,
		cv::ADAPTIVE_THRESH_MEAN_C, cv::THRESH_BINARY_INV, 21, 2);//21, 7);//

	//cv::Mat imageBinary2;
	//cv::erode(imageBinary, imageBinary2, cv::Mat());
	//imageBinary2.copyTo(imageBinary); //vs thres=thres2;
	//double resize_scale = 0.25;
	//cv::resize(image_binary, image_binary, cv::Size(), resize_scale, resize_scale);
	////cv::namedWindow("binary", cv::WINDOW_NORMAL);
	//cv::imshow("binary", image_binary);
	//cv::waitKey();
	//////////double resize_scale = 0.125;
	//////////cv::Mat img_left_resized, img_right_resized;
	//////////cv::resize(leftCopy, img_left_resized, cv::Size(), resize_scale, resize_scale);
	//////////cv::resize(rightCopy, img_right_resized, cv::Size(), resize_scale, resize_scale);
...
void detectRectangles(const cv::Mat& thresImg, std::vector<std::vector<cv::Point>>& OutMarkerContours)
{
	...
	for (unsigned int i = 0; i < contours2.size(); i++)
	{
		...
		//cv::resize(input, input, cv::Size(), 0.125, 0.125);
	...
	//cv::Mat input;
	//cv::cvtColor(thresImg, input, cv::COLOR_GRAY2BGR);
	//drawContour(input, MarkerCanditates[0], cv::Scalar(255, 0, 0));
	//cv::namedWindow("input");
	//cv::resize(input, input, cv::Size(), 0.125, 0.125);
	//cv::imshow("input", input);
	//cv::waitKey(10);
...
//     removeElements ( MarkerCanditates,toRemove );
	...
	//cv::Mat input;
	//cv::cvtColor(thresImg, input, cv::COLOR_GRAY2BGR);
	////drawContour(input, OutMarkerContours, cv::Scalar(255, 0, 0));
	//int index = 0;
	//for (index = 0; index < OutMarkerContours.size(); index ++)
	//{
	//	cv::Scalar color(rand() & 255, rand() & 255, rand() & 255);
	//	drawContours(input, OutMarkerContours, index, color, 15, 8);
	//}
	////drawContours(input, OutMarkerContours, index, color, CV_FILLED, 8, hierarchy);
	//cv::namedWindow("input");
...
bool distinguish8Points(const std::vector<cv::Point2f>& pointsIn, PatternContainer& pointsOut) {
	...
	for (const auto& a : pointsIn) {
		for (const auto& b : pointsIn) {
			...
			// find b(2) and c(3)
			float dis_ab = 10000, dis_ac = 10000;
			cv::Point2f p2, p3;
			//int cri_x_num = 0;
			for (const auto& c : pointsIn) {
				if (c == a) {
...
void markerIdentify(const cv::Mat& leftSrc, const cv::Mat& rightSrc, 
	...
	//identifyMarkerIdV2(leftPoints, cnr_rect_l,leftMarkerId_temp, leftMarkerPoints_temp, registrated_pcs);
	//identifyMarkerIdV2(rightPoints, cnr_rect_r, rightMarkerId_temp, rightMarkerPoints_temp, registrated_pcs);
...
bool getPoseFromPicture(const cv::Mat& leftSrc, const cv::Mat& rightSrc, cv::Mat& leftDst, cv::Mat& rightDst, cv::Mat& pose, float& residue)
{
	return false;
}
