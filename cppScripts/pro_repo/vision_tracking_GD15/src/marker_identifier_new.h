...
bool distinguish8PointsV2(const std::vector<cv::Point2f>& pointsIn, const std::vector<std::vector<cv::Point>> cnr_rect,
	PatternContainer& pointsOut, int& Pattern_ID);

bool IsPointInRect(cv::Point2f P, cv::Point2f A, cv::Point2f B, cv::Point2f C, cv::Point2f D = cv::Point2f(0.f, 0.f));

void identifyMarkerIdV2(const std::vector<std::vector<cv::Point2f>>& points8, const std::vector<std::vector<cv::Point>> cnr_rect,
...
bool getPoseFromPicture(const cv::Mat& leftSrc, const cv::Mat& rightSrc, cv::Mat& leftDst, cv::Mat& rightDst,
	cv::Mat& pose, float& residue);