...
std::tuple<CornersSorted, CornersSorted> Optimizator::flipCornersWhenNecessary(const CornersSorted& left, const CornersSorted& right)
{
    ...
    {
        std::vector<float> sepe_arr;
        for (int i = 0; i < rightSorted.size() - 1; i++)
        {
            sepe_arr.push_back(rightSorted.at(i).sepeNum);
        }

        reverse(rightSorted.begin(), rightSorted.end());
        reverse(sepe_arr.begin(), sepe_arr.end());

        for (int i = 0; i < rightSorted.size() - 1; i++)
        {
            rightSorted.at(i).sepeNum = sepe_arr.at(i);
        }
        rightSorted.at(rightSorted.size() - 1).sepeNum = 0;
    }
