...
void bugRecords::createWhichPDFSlots()
{
	switch (pdf_flag)
	{
	case 1:CreatePdf1("./data/tele_acc.log"); break;		//主从操作准确度和重复性测试报告
	case 2:CreatePdf2("./data/tele_acc.log"); break;
	case 3:CreatePdf3("./data/tele_acc.log"); break;
	case 4:CreatePdf4("./data/tele_acc.log"); break;
	case 5:CreatePdf5("./data/tele_acc.log"); break;
	}
	pdf_flag++;
	if (pdf_flag == 4)
		pdf_flag = 1;
...
void bugRecords::CreatePdf1(QString str ="./data/tele_acc.log")
{
...
void bugRecords::CreatePdf2(QString str)
{

	QFile file(str);

	if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
		QMessageBox::warning(this, "警告", "无实验结果");
		return;
	}
	//保存程序当前路径
	QString currentDir = QDir::currentPath();
	...
	if (1)
	{
		...
		pPdfPainter->setFont(font[2]);
		charti1++;
		pPdfPainter->drawText(QRect(chartL1 + 10, chartRowH1 * charti1 + chartT1 + 10, pdfWidth / 2, chartRowH1), Qt::AlignLeft | Qt::AlignVCenter, QString("产品型号 ：%1").arg(QString::fromStdString("SR-ENT-SP0802S")));
		charti1++;
		pPdfPainter->drawText(QRect(chartL1 + 10, chartRowH1 * charti1 + chartT1 + 10, pdfWidth / 2, chartRowH1), Qt::AlignLeft | Qt::AlignVCenter, QString("序 列 号 ：%1").arg(QString::fromStdString(" ")));
		charti1++;
		pPdfPainter->drawText(QRect(chartL1 + 10, chartRowH1 * charti1 + chartT1 + 10, pdfWidth / 2, chartRowH1), Qt::AlignLeft | Qt::AlignVCenter, QString("生产批号 ：%1").arg(QString::fromStdString(" ")));
...
void bugRecords::CreatePdf3(QString str)
{
	QFile file(str);
...
void bugRecords::CreatePdf4(QString)
{
	std::cout << "44444" << std::endl;
}

void bugRecords::CreatePdf5(QString)
{
