...
void ImageDetection::sortBubble(std::vector<PATTERN_CONTAINER>& marker_detected) {
	PATTERN_CONTAINER temp;//��ʱ����
	bool flag;//�Ƿ���������ı�־
	int length = marker_detected.size()-1;
	if (length < 1)
		return;
	for (size_t i = 0; i < length; i++) { //��ʾ������һ����Ҫn-1��
		for (size_t j = 0; j < length - i; j++) {
			flag = false;//ÿ�α���������flagΪfalse�������жϺ���Ԫ���Ƿ�������
			if (marker_detected[j].marker_pixel_id > marker_detected[j + 1].marker_pixel_id) {
				...
				marker_detected[j + 1] = temp;//ʵ�ֽ��������������ŵ����һλ
				flag = true;//ֻҪ��������flag������Ϊtrue
			}
		}
		if (!flag)
			break;//���flag�ڱ���ѭ����ʼ��Ϊfalse��˵������Ԫ���Ѿ���������ѭ������
	}
}