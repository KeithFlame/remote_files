...
devices_num(0){
	...
	// 线程对象实例化
	m_thread_leftcamera = new CameraThread;  //左相机线程对象
	m_thread_rightcamera = new CameraThread; //右相机线程对象

	// 图像指针实例化
	image_L = new cv::Mat();    // 图像指针实例化
	image_R = new cv::Mat();    // 图像指针实例化 
...
void CameraManipulation::OnBnClickedEnumButton(){
	...
	int nRet = MV_OK;
	nRet = CMvCamera::EnumDevices(MV_USB_DEVICE, &m_stDevList);             // ch:枚举子网内所有设备,相机设备数量
...
void CameraManipulation::CloseDevices(){
	...
	devices_num = CMvCamera::EnumDevices(MV_USB_DEVICE, &m_stDevList);           // ch:枚举子网内所有设备,相机设备数量
}
...
void CameraManipulation::OnBnClickedStartGrabbingButton(){
	...
	if (m_nTriggerMode == TRIGGER_ON){
		...
		for (unsigned int i = 0; i < m_stDevList.nDeviceNum; i++){
			...
			camera_Index = i;
			if (camera_Index == 0){
				m_thread_leftcamera->getCameraPtr(m_stereocamera[0]); //线程获取左相机指针
				m_thread_leftcamera->getImagePtr(image_L);  //线程获取左图像指针
				m_thread_leftcamera->setCameraSymbol(0); //不检测角点 ==0

				if (!m_thread_leftcamera->isRunning()){
					m_thread_leftcamera->start();
					m_stereocamera[0]->softTrigger();
					m_stereocamera[0]->ReadBuffer(*image_L);//读取Mat格式的图像
				}

			}

			if (camera_Index == 1){
				m_thread_rightcamera->getCameraPtr(m_stereocamera[1]); //线程获取右相机指针
				m_thread_rightcamera->getImagePtr(image_R);   //线程获取右图像指针
				m_thread_rightcamera->setCameraSymbol(0); //右相机 Index==1

				if (!m_thread_rightcamera->isRunning()){
					m_thread_rightcamera->start();
					m_stereocamera[1]->softTrigger();
					m_stereocamera[1]->ReadBuffer(*image_R);//读取Mat格式的图像
				}
			}
		}
	...
	if (m_nTriggerMode == TRIGGER_OFF){
		...
		for (unsigned int i = 0; i < m_stDevList.nDeviceNum; i++){
			...
			if (i == 0){
				...
				displayLeftImage(image_L, i);//左相机图像
			}
			if (i == 1){			//开启相机采集
				nRet = m_stereocamera[i]->CommandExecute("TriggerSoftware");
...
void CameraManipulation::SaveImage(){
	...
	for (int i = 0; i < devices_num; i++){
		...
		if (NULL == m_stereocamera[i]->m_pBufForDriver){
			...
			m_stereocamera[i]->m_nBufSizeForDriver = nRecvBufSize;  // 一帧数据大小
		...
		if (MV_OK == nRet){
			...
			stParam.nBufferSize = m_stereocamera[i]->m_nBufSizeForSaveImage;  // 存储节点的大小 | en:Buffer node size
...
bool CameraManipulation::isExists(std::string file_path){
	std::string dir = "./"+ file_path; //文件夹路径
