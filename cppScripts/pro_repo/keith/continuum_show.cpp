...
namespace math_c
{
    float ang2deg(float ang)
    {
        return ang * PI / 180.f;
    }
    float deg2ang(float deg)
    {
        return deg / PI * 180.f;
    }
}
...
void ContinuumShow::drawCircle(float z_f = 0.f)
{
    ...
    for (size_t i = 0; i <= 360 + (int)(tp); i += (int)tp)
    {
        float p = i * 3.14 / 180;
...
void ContinuumShow::drawCylinder(float z_f = 1.f)
{
    ...
    {
        p = i * PI / 180;
        glVertex3f(sin(p) * r, cos(p) * r, z_f + sp);
        glVertex3f(sin(p) * r, cos(p) * r, z_f + 0.f);
    }
    glColor4f(1.f, 0.f, 0.f,0.3f);
    glEnd();
    drawCircle(0.f + z_f);
...
void ContinuumShow::drawContinuum()
{
    Eigen::Matrix3f mat;
    float theta = PI / 12;
    float delta = PI / 2;
    ...
    mat <<powf( cosdelta , 2) * costheta + powf(sindelta , 2), - sindelta * cosdelta * (costheta - 1), cosdelta * sintheta,
        sindelta* cosdelta* (1 - costheta), powf(cosdelta , 2) + costheta * powf(sindelta , 2), - sindelta * sintheta,
        - cosdelta * sintheta, sindelta * sintheta, costheta;
    Eigen::Vector3f pos = Eigen::Vector3f{0.f, 0.f, -4.f};
    
    drawContinuumSpacer(mat, pos);
    drawContinuumSpacer(mat * mat, pos * 2);
    drawContinuumSpacer(mat * mat * mat, pos * 3);
}

void ContinuumShow::drawContinuumSpacer(Eigen::Matrix3f mat, Eigen::Vector3f P)
{
    Eigen::AngleAxisf axang(mat);
    Eigen::Vector3f vec = axang.axis();
    float ang = axang.angle();

    glTranslatef(P(0), P(1), P(2));
    glRotatef(math_c::deg2ang(ang), vec(0), vec(1), vec(2));
