...
%
% include procximal strain and half-stiffness segment bending
% Author: Keith W.
% Ver. 1.0
% Date: 08.10.2022
...
clear all;close,clc;
%% 基础知识
psi = [40 0 90 0 0 0 45]';
psi(2:7)=psi(2:7)*pi/180;

is_plot = 1;
arm_serial = getArmPara(112,4);
Fe=[5 0 0]';Me=[0 0 0]';
...
function qa = fC2M(u,MP)
    ...
    dGamma(1:2,1:3)=(MP.L1+MP.L0*MP.gamma)*MP.Q1';
    dGamma(3:4,1:3)=(MP.L1+MP.L0*MP.gamma)*MP.Q2';
    dGamma(3:4,4:6)=MP.L2*MP.Q2';
    dGamma(3:4,7:9)=-MP.alpha*MP.L3*MP.Q2';
    dGamma(5:6,7:9)=-MP.L3*MP.Q3';

    Ell(1:2,1:2)=diag([1 1])*(MP.L1+MP.L);
    Ell(3:4,3:4)=diag([1 1])*(MP.L2+MP.Lr+MP.L1+MP.L+MP.Lcnla +MP.L3);
    Ell(5:6,5:6)=diag([1 1])*MP.L3;

    Theta(1:3,1:2)=-MP.A1*MP.E*MP.Q1;
    Theta(4:6,3:4)=-MP.A2*MP.E*MP.Q2;
    Theta(7:9,5:6)=-MP.A3*MP.E*MP.Q3;
    ...
    K(4:6,4:6)=16*MP.Kb2 + 4 * MP.Kb3;

    uc=u;
    u1=uc(1:3)';u2=uc(4:6)';
    u3=pinv(MP.Q2')/MP.alpha/MP.L3*( MP.Q2'*(MP.L1+MP.gamma*MP.L0)*u1+...
        (MP.Q2'*MP.L2+ (MP.L2+MP.Lr+MP.LL1*1e-3+MP.L+MP.Lcnla+MP.L3)/MP.A2/MP.E*pinv(MP.Q2)*...   %x(1)*1e-3
...
function MP = getMP(arm_serial, l1)
    ...
    MP.r31=[1 0 0]'*MP.rho3;
    MP.r32=[0 1 0]'*MP.rho3; 
...
%     P1 = P1*1e3;P2 = P2*1e3;
    ...
    % 画间隔片和刚性段
    step_2 = arm_serial.assembly_para(8)*(N2-2)/arm_serial.size_para(3);
    ...
    step_1 = arm_serial.assembly_para(8)*(N1-2)/L1;
