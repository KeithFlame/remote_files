...
T_needle = eye(4);T_needle(1:3,1:3)=eul2rotm([0 0/180*pi 0]);T_needle(1:3,4) = [50 -30 110];
R1=eul2rotm([0/180*pi 0 0]);
P1=R1*eul2rotm([90/180*pi 0 0])*[-SP.needle.r;0; 0];R1=R1*eul2rotm([0 0/180*pi 30*pi/180]);
R2=eul2rotm([70/180*pi 0 0]);
P2=R2*eul2rotm([90/180*pi 0 0])*[-SP.needle.r;0; 0];R2=R2*eul2rotm([0 0/180*pi -30*pi/180]);
R4=eul2rotm([-70/180*pi 0 0]);
P4=R4*eul2rotm([90/180*pi 0 0])*[-SP.needle.r;0; 0]; R4=R4*eul2rotm([0 0/180*pi -30*pi/180]);
T1_0=T_needle*[R1 P1;[0 0 0 1]];
T2_0 = T_needle*[R2 P2;[0 0 0 1]];
T4_0 = T_needle*[R4 P4;[0 0 0 1]];
...
psi2=[psi2 pi/45];
psi4=[psi4 pi/45];
psi3=[ 40 pi/2 pi/6 pi/2 pi/2 -pi/2 ];
