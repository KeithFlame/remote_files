function [Tend, S] = FKco_2segs_bending_sr_keith(ua, MBP, FM, discrete_element)
% Declaration
...
%% Execution
% ua = ua(3:end);
[~,t1,t2,y1,y2]=shootingOpt(ua,Fe,Me,fe,le,MBP);
...
%% Model Functions
function [Guess,t1,t2,y1,y2]=shootingOpt(ua,Fe,Me,fe,le,MBP)
    dGuess=eye(6)*1e-5;
    lambda=5e-10; % Jacobian damping
    eps=1e-5; % residue tolerance
    
    nc=[0 0 0]';mc=[0 0 0]';v=[0 0 0 0]';Guess=[nc;mc]; % guessed base force moment strain
    [Rsd,t1,y1,t2,y2]=forShooting(Guess,ua,Fe,Me,fe,le,MBP);
    
    J= zeros(6,6);
    while(norm(Rsd)>eps)    
        for i=1:6 % finite differencing for Jacobian of initial guess
            [Rsd_]=forShooting(Guess+dGuess(:,i),ua,Fe,Me,fe,le,MBP);
            J(:,i)=(Rsd_-Rsd)/norm(dGuess(:,i));
        end
    
        Guess=Guess-eye(6)/(J'*J+lambda*eye(6))*J'*(Rsd); % update guess
        [Rsd,t1,y1,t2,y2]=forShooting(Guess,ua,Fe,Me,fe,le,MBP); % number IVP of equal to number of guessed values 
    
    end

end

function [Rsd,t1,y1,t2,y2]=forShooting(Guess,ua,Fe,Me,fe,le,MBP)
% do an IVP shooting
...
v=zeros(3,4);
% v(end,:)=Guess(7:10)'; % rod elongation strain

p0=zeros(3,1);R0=eul2rotm([ua(1) 0 0]);
y0=[p0' R0(:,1)' R0(:,2)' R0(:,3)' nc' mc' 0 0 0 0]';
ua1=ua(3:5);ua2=ua(6:8);ua0=zeros(3,1);

if(MBP.Ls)>0
    [t0,y0]=odeCosserat1(y0,v,0,MBP,fe,le,MBP.discrete_element,ua0);
    y0_=y0(end,:);
else
    t0 = 0;
    y0_=y0;
...
[t1,y1]=odeCosserat1(y0_,v,1,MBP,fe,le,MBP.discrete_element,ua1); % first seg y=p,R,n,m,q for each row
...
y1_=[yL1(1:3);yL1(4:15);cross(R1*[0 0 -MBP.Lr]',yL1(13:15)) + yL1(16:18)]; % ? start of 2nd seg

[t2,y2]=odeCosserat1(y1_,v,2,MBP,fe,le,MBP.discrete_element,ua2); % second seg
...
Rsd(4:6)=Me-yL2(16:18) - cross(R2*[0 0 -MBP.Lg]',yL2(13:15));
end
...
function [t,y,U]=odeCosserat1(y0,v,SegIdx,MBP,fe,le,step,ua)
...
Kb2=MBP.Kb2;
Ke2=MBP.Ke2;
I1 = 2*MBP.I1+pi*MBP.d1^2/4*MBP.rho1^2*0;
I2 = 2*MBP.I2+pi*MBP.d2^2/4*MBP.rho2^2*0;
Kb01 = diag([MBP.E*I1 MBP.E*I1 2*I1*MBP.G]);
Kb02 = diag([MBP.E*I2 MBP.E*I2 2*I2*MBP.G]);
if(SegIdx == 1)
    Q=MBP.Q1;
    N=ceil(MBP.L1o/step);
    t=linspace(0,MBP.L1o,N)';
    mod_SegIdx=1;
    Kb = (4*Kb01+16*Kb02)/2+MBP.K1*Kb1;
    step=MBP.L1o/(N-1);
elseif(SegIdx == 2)
    Q=MBP.Q2;
    N=ceil(MBP.L2/step);
