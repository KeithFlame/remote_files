function [Rsd,MBP,t0,y0,t1,y1,t2,y2,t3,y3,QA,ee_all]=forInvShooting_keith(Guess,T,ksi,MBP)
% do an IVP shooting
MBP1 = MBP(1); MBP2 = MBP(2);
ksi1= ksi(1:5); ksi2 = ksi(6:10);
pz1=Guess(8);
pz2 = Guess(20);
MBP1 = MBP1.resetK1(ksi(11));
MBP2 = MBP2.resetK1(ksi(12));
MBP1 = MBP1.refreshLso(pz1*1000);
MBP2 = MBP2.refreshLso(pz2*1000);
MBP = [MBP1 MBP2];

vec61=[-10.727 97.202 82.201 81.055 54.728 -50.440]';
T10 =fromX2T(vec61);T10(1:3,4)=T10(1:3,4)/1000;
vec62=[0.091 96.565 81.745 83.186 41.881 -40.445]';
T20 =fromX2T(vec62);T20(1:3,4)=T20(1:3,4)/1000;
dr11 = [-28.226 114.521 82.452];
dr11 = fromaxang2rotm(dr11);
dr12=[10.583 130.792 95.750];
dr12 = fromaxang2rotm(dr12);
dr21=[54.634 -74.704 47.909];
dr21 = fromaxang2rotm(dr21);
dr22=[-81.868 -1.600 -34.488];
dr22 = fromaxang2rotm(dr22);
% arm1
nc1=Guess(1:3); % base internal force
mc1=Guess(4:6); % base internal load
Rz1 = Guess(7);
v1=zeros(3,4);v1(end,:)=Guess(9:12)'; % rod elongation strain
...
T10(1:3,1:3)=T10(1:3,1:3)*eul2rotm([Rz1 0 0]);
...
%% arm2
nc2=Guess(13:15); % base internal force
mc2=Guess(16:18); % base internal load
Rz2=Guess(19);
v2=zeros(3,4);v2(end,:)=Guess(21:24)'; % rod elongation strain
...
T20(1:3,1:3)=T20(1:3,1:3)*eul2rotm([Rz2 0 0]);
...
R23 = R23*dr22;
Rsd(16:18)=invSkewMatrix_keith(Rrsd*R23'-R23*Rrsd');

QA = zeros(12,1);
QA(1:6)=[Guess(7:8)+[0 0.00]';yL11_(19:20)-qe11;yL12_(19:20)-qe12];
QA(7:12)=[Guess(19:20)+[0 0.00]';yL21_(19:20)-qe21;yL22_(19:20)-qe22];
end