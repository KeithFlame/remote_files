...
# def draw_point(image, uv, is_left=True):              848.34 575.26
#    
#     u = uv[0]
#    v = uv[1]
#
...
def draw_cor(image, trans, offset=0, K=K_left_new):
    ...
    if len1>123:
        trans2 = trans[122:len1-1][:]
        for i in range(1, len1-124):
            cv.line(image, (int(trans2[i][0]),int(trans2[i][1])), (int(trans2[i+1][0]),int(trans2[i+1][1])), (0, 0, 255), 4)
        
        cv.circle(image, (int(trans[len1-1][0]),int(trans[len1-1][1])), 15, (0, 0, 255), -1)
    # print(trans[len1-1][:])
    

    cv.circle(image, (848,575), 15, (0, 255, 255), -1)
...
def read_log(file_name, num=161):
    t = np.zeros([num, 2])
