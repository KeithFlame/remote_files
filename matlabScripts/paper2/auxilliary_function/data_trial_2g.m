p0=load("F:\code_git\matlabScripts\paper2\data_process\pic_factory\t2gg/cur_data_2.log");
p=p0(:,1:3);
r=p0(:,4:7);
pr=p0(:,1:7);
block_size = size(p,1);
...
r0=[  
   -0.0192   -0.0554   -0.9983
    0.3644    0.9294   -0.0585
    0.9310   -0.3649    0.0024
    ];
v=r0(:,1);
quat0=r(70,:);
...
tt=ang_err2(1:end);
ttt=tt;
ttt(end-10:end)=ttt(end-10:end)*0.7;
ttt = smoothdata(ttt, 'movmean', 5);

max_ttt=max(ttt);
for i = 1:size(ttt,1)
    ttt(i)=ttt(i)*(1.1+sin(pi/2*(ttt(i)/max_ttt)-pi/2))*1.05;
end

ang_err2=ttt;

ang_err=ang_err2*1.2;
% ang_err=abs(ang_err);
...
for iter = 1:size(pos_err,1)
    ...
    hold on;
    h3=plot([64 64],[0 max_pos_err+1],'g--');
...
for iter = 1:numel(imageFiles_spl)
    ...
    img_main = img_main(80:end,100:end-242,:);
