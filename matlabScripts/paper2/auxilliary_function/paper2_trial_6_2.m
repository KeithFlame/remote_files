p0=load("F:\code_git\matlabScripts\paper2\data_process\pic_factory\trial_6_2/cur_data_2.log");
...
window_size = 5;
quat0 = [0.4829   -0.5508   -0.5370    0.4185];
p0 = [10 4 54];
...
Target = [r0 p0';[0 0 0 1]];Target(2,4)=Target(2,4)-1;
...
for iter = 1:dis_b%numel(imageFiles_spl)
    ...
    % height = 2*size(img_main,1);%+size(img_aux,1);
    % pln_ratio = height / pln_h;
    % spl_ratio = height / spl_h;
    if(iter==987)
        break;
    end
    % 计算缩放比例
