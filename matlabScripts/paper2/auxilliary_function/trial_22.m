p0=load("F:\code_git\matlabScripts\paper2\data_process\pic_factory\test_05271/cur_data.log");
p=p0(:,1:3);
r=p0(:,4:7);
pr=p0(:,1:7);
block_size = size(p,1);
...
r0=[  
    0.0008   -0.0005   -1.0000
   -0.8659    0.5002   -0.0009
    0.5002    0.8659   -0.0000
    ];
v=r0(:,1);
quat0=r(70,:);
...
p1=[-15 3 70];p2=[-15 5 90];
e4=(p2-p1)/norm(p2-p1);
p40=p1;
x=[p1(1) p2(1)];y=[p1(2) p2(2)];z=[p1(3) p2(3)];
line(x, y,z, 'Color', 'blue', 'LineWidth', 2);

p1=[-15 5 90];p2=[-10 5 90];
...
for iter = 1:size(pos_err,1)
    ...
    hold on;
    h3=plot([40 40],[0 max_pos_err+1],'g--');
...
for iter = 1:numel(imageFiles_spl)
    ...
    img_aux = img_aux(50:end-50,50:end-580,:);
