function PS_2segs_keith(S, SL, Tend, MBP, T_tip, discrete_element, init_pose, plot_type)
% Declaration
...
s5 = S(1:3, n0+n1+nr+n2:n0+n1+nr+n2+ng);
    ...
    if(plot_type == 2)
        ...
        for i = 1:number_niti_L1
            ...
            delta_angle= MBP.delta_t1 + first_niti_L1 + interval_angle*(i-1);
            p_i=s2;
    ...
    % 画超声刀杆
    P_IAUS = [T_tip(1:3,4) MBP.b_P_port*1e3];
    plot3(P_IAUS(1,:),P_IAUS(2,:),P_IAUS(3,:),LineWidth=2,Color='black');
    plotCoord_keith(T_tip,1,discrete_element);
    hold off;    
end
