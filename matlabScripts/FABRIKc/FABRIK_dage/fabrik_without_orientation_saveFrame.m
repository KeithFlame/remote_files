...
%     F=getframe(gcf);
%     tmp=F;
%     filename=['D:\RII\项目\5.fabrik_for_continuum\论文\media\algorithm\frame_',num2str(frame_num),'.png'];
...
while error(end)>=1e-2
    ...
    target_theta=sign(dot((target-pc(n).virtual_joint)/norm(target-pc(n).virtual_joint),[0;1]))*acos(dot((target-pc(n).virtual_joint)/norm(target-pc(n).virtual_joint),[0;1]));
    ...
    joint=scatter(joint_temp(1),joint_temp(2),100,'MarkerEdgeColor',[.5,.5,0]);
    l1=line([target(1) joint_temp(1)],[target(2) joint_temp(2)],'LineStyle','--','LineWidth',2);
    pause(0.1);
...
%     frame_num=frame_num+1;
    ...
    l2=line([joint_temp(1), pc(n-1).virtual_joint(1)],[joint_temp(2), pc(n-1).virtual_joint(2)],'LineStyle','--','LineWidth',2);
    pause(0.1);
...
%     frame_num=frame_num+1;
    ...
    joint=scatter(joint_temp(1),joint_temp(2),100,'MarkerEdgeColor',[.5,.5,0]);
    l1=line([pc(n+1).endtip(1), joint_temp(1)],[pc(n+1).endtip(2), joint_temp(2)],'LineStyle','--','LineWidth',2);
    pause(0.1);
...
% %     theta_dis=acos(dot(pc(n-1).endrotation(:,2),target-joint_temp)/norm(target-joint_temp));
        ...
        target_theta=sign(dot((target-joint_temp)/norm(target-joint_temp),[0;1]))*acos(dot((target-joint_temp)/norm(target-joint_temp),[0;1]));
    joint=scatter(joint_temp(1),joint_temp(2),100,'MarkerEdgeColor',[.5,.5,0]);
    l1=line([pc(n-1).endtip(1), joint_temp(1)],[pc(n-1).endtip(2), joint_temp(2)],'LineStyle','--','LineWidth',2);
    pause(0.1);
