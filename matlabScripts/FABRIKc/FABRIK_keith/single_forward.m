function mu = single_forward(motion_units,i,hd,is_plot)
%MOTION_UNIT 此处显示有关此函数的摘要
%   此处显示详细说明
%   运动单元建立

    ee1 = motion_units(i+1).e1;
    motion_units(i).joint_position = motion_units(i+1).origin_position + ...
        ee1 * motion_units(i).l2;
    if((i-1) ~= 1 && motion_units(i-1).joint_type == 0)
...
%         else
            l1 = norm(motion_units(i-1).joint_position - ...
                motion_units(i).joint_position) * ee2'*ee1 - motion_units(i-1).l2;
%         end
...
%         motion_units(i).origin_position = motion_units(i).joint_position + ...
%             motion_units(i).l1 * ee2;
        theta = acosd(ee2'*ee1);
        if(theta>180)
            ee3 = cross(ee2,ee1);
            ee3 = cross(ee1,ee3);
            ee3 = ee3/norm(ee3)/sqrt(3) + ee1;
            ee3 = ee3/norm(ee3);
            motion_units(i).origin_position = motion_units(i).joint_position + ...
