function [flag, time,iteration,error_PR,THETA]=cFABRIK_7DoF_without_Broyden(target,LS,initial_pose,err_epsilon,kmax,limit)
...
while k < k_max && error_current > error_P_desired
    ...
    if(is_plot)
        ...
        [XX1, YY1] = meshgrid((P2(1)-50):10:(P2(1)+50), (P2(2)-50):10:(P2(2)+50));
        A = P2;N = N1;
        if(abs(N(3))>1e-6)
            ZZ1 = A(3) + (N(1)*(XX1 - A(1)) + N(2)*(YY1 - A(2))) / - N(3);
            set(hs_for, 'XData', XX1);set(hs_for, 'YData', YY1);set(hs_for, 'ZData', ZZ1);
        end
    ...
    if(is_plot)
        set(h3, 'XData', [P2(1) P3(1)]);set(h3, 'YData', [P2(2) P3(2)]);set(h3, 'ZData', [P2(3) P3(3)]);
        [XX1, YY1] = meshgrid((P3(1)-50):10:(P3(1)+50), (P3(2)-50):10:(P3(2)+50));
        A = P3;N = N2;
        if(abs(N(3))>1e-6)
            ZZ1 = A(3) + (N(1)*(XX1 - A(1)) + N(2)*(YY1 - A(2))) / - N(3);
            set(hs_back, 'XData', XX1);set(hs_back, 'YData', YY1);set(hs_back, 'ZData', ZZ1);
        end
