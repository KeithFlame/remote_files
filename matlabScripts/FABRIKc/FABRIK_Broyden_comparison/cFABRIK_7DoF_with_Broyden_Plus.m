function [flag, time,iteration,error_PR,THETA]=cFABRIK_7DoF_with_Broyden_Plus(target,LS,initial_pose,err_epsilon,kmax,limit)
...
while k < k_max && error_current > error_P_desired
    k = k + 1;
    theta_center(:,1) = theta_center(:,2);
    theta_center(:,2) = theta_center(:,3);
    theta_center(:,3) = [theta1 theta2 theta3 theta4]';
    % Broyden
    if(k>5)
        ...
        if(k>10)
            ...
            sum_theta = sum(theta_center');
            cter = sum_theta - limit;
            if(cter(4) == 0)
                P3 = P3 + rand(3,1);
            else
