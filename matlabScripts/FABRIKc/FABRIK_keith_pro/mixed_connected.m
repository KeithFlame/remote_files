...
joint_unit5dd = MotionUnit(P(:,6), (P(:,5) + P(:,6))/2,P(:,5), P(:,6)+ e2, P(:,5) + e1, joint_units_type(5));
...
joint_unit10 = MotionUnit(P(:,10), (9999*P(:,9) + P(:,10))/10000,P(:,9), P(:,10) + e1, P(:,9) + e2, joint_units_type(1));
...
while(errp>err_p)
    ...
    for i = (block_size - 1) :-1: 2
        ...
        ju = single_forward_pro(joint_units3,i,hd,is_plot, errp);
        joint_units3(i) = ju;
        if(is_write)
            [I,map] = rgb2ind(frame2im(getframe(gcf)),128);
            imwrite(I,map,'test.gif','WriteMode','append','DelayTime',dt);
    ...
    for i = 2 : (block_size - 1)
        ...
        ju = single_backward_pro(joint_units3,i,hd,is_plot);
        joint_units3(i) = ju;
        if(is_write)
            [I,map] = rgb2ind(frame2im(getframe(gcf)),128);
            imwrite(I,map,'test.gif','WriteMode','append','DelayTime',dt);
    ...
    joint_units3(end).origin_position = joint_units(end).joint_position;
    joint_units3(end).origin_direction_position = joint_units(end).e1;
    
    e00 = joint_units3(end - 1).joint_position - joint_units(end).joint_position;
    e00 = e00 / norm(e00);
    joint_units3(end).joint_position = joint_units(end).origin_position + ...
        joint_units(end).l1 * e00;
    joint_units3(end).end_position = joint_units(end).joint_position + ...
        joint_units(end).l2 * e00;
    joint_units3(end).end_direction_position = joint_units(end).e1;
    joint_units3(end) = joint_units3(end).updateUnit; 
