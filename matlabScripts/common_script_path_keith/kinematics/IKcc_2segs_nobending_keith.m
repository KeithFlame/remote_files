function [psi_inv, flag_psi] = IKcc_2segs_nobending_keith(T, SL, psi_limit)
% Declaration
...
% 
% input1: target pose T (4 X 4 matrix)
% input2: structural length SL (4 X 1 vector, L1 Lr L2 Lg)
%                             or (7 X 1 vector additional gamma1 gamma2 gamma3)
% input3: psi limit (6 X 2 matrix,  each raw for each configeration variable)
...
% Date: 09.02.2023
    ...
    if (nargin == 2)
        psi_limit = [-pi pi; 0 200; 0 pi/2; -pi pi;0 pi/3*2; -pi pi];
    end
    L1=SL(1);Lr=SL(2);L2=SL(3);Lg=SL(4);
    config=invKineC3(T,Lr,L2,Lg);
    psi_inv=[config(1) config(4) config(3) config(5) config(7) config(9)]';
    flag_psi = 1;
    if(config(4)>L1)
        config=invKineC4(T,L1,Lr,L2,Lg);
        psi_inv=[config(1) config(2)+L1 config(3) config(5) config(7) config(9)]';
        flag_psi = 2;
    end
    t1 = min(psi_inv - psi_limit(:,1));
    t2 = min(psi_limit(:,2) - psi_inv);
    if(t1<0||t2<0)
        [psi_inv, flag_psi_test] = limitRange(psi_inv, psi_limit);
        if(flag_psi_test < 700)
...
% for psi limit
function [psi_l, flag_psi] = limitRange(psi, psi_limit)
    psi_l=psi;
    joint_limit = reshape(psi_limit',[12 1]);
    flag_psi = 800; % means no-valid symbol
...
%     end
    while(psi(4)<joint_limit(7)||psi(4)>joint_limit(8))
