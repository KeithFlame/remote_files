function [t,y]=odeCosserat_inverse_fix_twist(y0,v,SegIdx,MP,fe,le,step,uz_fix)
...
for i=1:N-1
    ...
    if(theta~=0)
        ...
        R_dot=[cos(delta)^2*cos(theta)+sin(delta)^2 -sin(delta)*cos(delta)*(cos(theta)-1) cos(delta)*sin(theta);...
              ...
              -cos(delta)*sin(theta) sin(delta)*sin(theta) cos(theta)]*Expm(-[0 0 1]'*u(3)*(t(i)-t(i+1)));
    else
        p_dot=R*[0 0 -(t(i)-t(i+1))]';
        R_dot=eye(3)*Expm(-[0 0 1]'*u(3)*(t(i)-t(i+1)));
    end
    p=p+(p_dot);
    R=R*R_dot';
