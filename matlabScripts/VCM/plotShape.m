...
hD1=drawCircle(p(N1-1,:),[Rs(N1-1,1:3);Rs(N1-1,4:6);Rs(N1-1,7:9)]',MP.rho+0.15*1e-3,color);
if(N2~=0)
hD2=drawCircle(p(N1,:),[Rs(N1,1:3);Rs(N1,4:6);Rs(N1,7:9)]',MP.rho+0.15*1e-3,color);
hD3=drawCircle(p(N1+N2-1,:),[Rs(N1+N2-1,1:3);Rs(N1+N2-1,4:6);Rs(N1+N2-1,7:9)]',MP.rho+0.15*1e-3,color);
else
    hD2=[];
    hD3=[];
end
